...
formElement.addEventListener('submit', function (e) {
    ...
    if (inputElement.value !== '') {
        ...
        // create an element that represents the text we hace to add (our to do) 
...
// clicking on a task allows you to toggle between checked/unchecked (AKA done vs not done)
    ...
    // NOTE: this will not work because you can only add event listeners to elements which exist in the DOM at the tieme of the code execution (the li elements will appear after the page load)
    // const listElements = document.querySelector('li').addEventListener();
...
// in order to attach a click event listener to the li's which do not exist on the page yet, we can use EVENT PROPAGATION to DELEGATE the click event to the ul!
